...
type WakeWordScreenProps = {
  onNextStepWordDetected: () => void; // Callback for handling wake word detection
};

const WakeWordScreen: React.FC<WakeWordScreenProps> = ({ onNextStepWordDetected }) => {
    ...
    const initializePorcupine = async () => {
      try {
        porcupineManagerRef.current = await PorcupineManager.fromKeywordPaths(
          accessKey,
          ["다음단계_ko_android_v3_0_0.ppn"], // '다음단계' 키워드 파일 경로
          () => {
            console.log("다음단계 detected!");
            onNextStepWordDetected(); // Trigger the callback function
          },
          (error) => {
  ...
  }, [onNextStepWordDetected]); // Dependency on onWakeWordDetected to avoid unnecessary effect executions
